...
Let see the partial ACF of residuals plot
```{r}
...
The temporal trend is now bordeline significant and the SD of parameters larger
...
Plot both model prediction. Trends are only marginally different, but the confidence intervals are larger when accounting for temporal autocorrelation. Hence the trends become non-significant at alpha=0.05.
```{r}
...
## Spatial structure is more complex than temporal; there are two dimensions
...
Then simulate some environmental variable correlated with latitude.
```{r}
#install.packages('ncf')
#library(ncf)
set.seed(2)
div.ncf=rmvn.spa(Long.x, Lat.y, 12, method = "exp", nugget = 2)# we call it div.nfc
...
```{r}
set.seed(2)
env.var=rnorm(101,3,1)# random environmental variable
```


And now simulate a diversity value responding to this environmental variable
We will add spatially structured noise by adding *'div.ncf'*
```{r}
#div=10+2.5*env.var-0.6*env.var^2+3*div.ncf 
...
Lets have a look at the variogram.
```{r}
